...
class TeacherReportController extends Controller
{
    ...
    private function getVisibleTeacherIds($user): array
    {
        ...
        if (in_array($user->role, ['therapist_homeroom', 'shadow_pj'])) {
            // Lihat guru yang mengajar murid yang sama di tahun ajaran aktif
            $myStudentIds = \App\Models\TeacherStudentPeriod::where('teacher_id', $user->id)
                ->where('academic_year', $academicYear)
                ->where('is_active', true)
                ->pluck('student_id');
 
            $peerIds = \App\Models\TeacherStudentPeriod::whereIn('student_id', $myStudentIds)
                ->where('academic_year', $academicYear)
                ->where('is_active', true)
                ->pluck('teacher_id')
                ->toArray();
